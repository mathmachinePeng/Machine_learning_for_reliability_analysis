...
from TAlogistic import LogisticRegression, load_data
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             dataset=[1,2,3], batch_size=3, n_hidden=50, testoutside=[1,2,3]):
    """
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    #-------------------------------------- #save the best model
                    #------------------------- f = file('simple_model.pkl', 'w')
                    # cPickle.dump(classifier, f, protocol=cPickle.HIGHEST_PROTOCOL)
                    #------------------------------------------------- f.close()
                    # Check the final predictions of all test data

                    #-------- test_model_final = theano.function(inputs=[index],
                            #----- outputs=classifier.logRegressionLayer.y_pred,
                            #------------------------------------------ givens={
                                #-------------------------------- x: test_set_x,
                                #----- y: test_set_y}, on_unused_input='ignore')
                    # outputtest = classifier.logRegressionLayer.y_pred.getvalue()
...
def predictt(test):
    ...
    classifier = cPickle.load('simple_model.pkl')
#    classifier = cPickle.load(open('best_model.pkl'))

    # compile a predictor function
    predict_model = theano.function(
        input=[classifier.inputs],
