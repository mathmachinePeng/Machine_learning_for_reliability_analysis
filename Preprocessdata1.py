...
def init_process(raw_data, ratio):
    X=np.array(raw_data)
...
def scaledivd(raw_data, ratio):
    a, b, aa, bb = init_process(raw_data, ratio)
    X=np.array(raw_data)
...
def normalscale(raw_data, ratio):
    X=np.array(raw_data)
    rows, dimens, new_rows, new_dimens = init_process(raw_data, ratio)
